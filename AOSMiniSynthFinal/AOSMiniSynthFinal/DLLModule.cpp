...
void CDLLModule::calculateShelfCoeffs(float fLowGain, float fHighGain)
{m_nSampleRate = 44100;
	// use same terms as in book:
	float theta_c_L = 2.0*pi*700/(float)m_nSampleRate; //hard code bass treble cutoffs
	float theta_c_H = 2.0*pi*4000/(float)m_nSampleRate;
	float d = 1.0;

	// intermediate values
		//low
	float fBetaNumerator_L =   4;
	float fBetaDenominator_L = 1.0 + pow(10,m_fBassGain/20);
		//high
	float fBetaNumerator_H =   1.0 - pow(10,m_fTrebleGain/20);
	float fBetaDenominator_H = 4;

	// beta
	float fBeta_L = 0.5*(fBetaNumerator_L/fBetaDenominator_L);	//low
	float fBeta_H = 0.5*(fBetaNumerator_H/fBetaDenominator_H); //high
	// alpha
	float fAlpha_L = fBeta_L * tan(theta_c_L/2); //low
	float fAlpha_H = fBeta_H * tan(theta_c_H/2); //high
	// gamma
	float fGamma_L = (1 - fAlpha_L)/(1 + fAlpha_L); //low
	float fGamma_H = (1 - fAlpha_H)/(1 + fAlpha_H);//high


	// left channel
	//low
	m_LeftShelf.m_f_a0 = (1-fGamma_L)/2;
	m_LeftShelf.m_f_a1 = (1-fGamma_L)/2;
	m_LeftShelf.m_f_a2 = 0.0;
	m_LeftShelf.m_f_b1 = -fGamma_L;
	m_LeftShelf.m_f_b2 = 0.0;
	m_LeftShelf.m_f_c0 = pow(10,m_fBassGain/20) -1.0;
	m_LeftShelf.m_f_d0 = 1.0;
	//high
	m_LeftShelf.m_f_a0 = (1+fGamma_H)/2;
	m_LeftShelf.m_f_a1 = -((1+fGamma_H)/2);
	m_LeftShelf.m_f_a2 = 0.0;
	m_LeftShelf.m_f_b1 = -fGamma_H;
	m_LeftShelf.m_f_b2 = 0.0;
	m_LeftShelf.m_f_c0 = pow(10,m_fTrebleGain/20) -1.0;
	m_LeftShelf.m_f_d0 = 1.0;

	// right channel
	//low
	m_RightShelf.m_f_a0 = (1-fGamma_L)/2;
	m_RightShelf.m_f_a1 = (1-fGamma_L)/2;
	m_RightShelf.m_f_a2 = 0.0;
	m_RightShelf.m_f_b1 = -fGamma_L;
	m_RightShelf.m_f_b2 = 0.0;
	m_RightShelf.m_f_c0 = pow(10,m_fBassGain/20) -1.0;
	m_RightShelf.m_f_d0 = 1.0;
	//high
	m_RightShelf.m_f_a0 = (1+fGamma_H)/2;
	m_RightShelf.m_f_a1 = -((1+fGamma_H)/2);
	m_RightShelf.m_f_a2 = 0.0;
	m_RightShelf.m_f_b1 = -fGamma_H;
	m_RightShelf.m_f_b2 = 0.0;
	m_RightShelf.m_f_c0 = pow(10,m_fTrebleGain/20) -1.0;
	m_RightShelf.m_f_d0 = 1.0;

}

void CDLLModule::cookVariables() //function to cook variables
	{
...
CDLLModule::CDLLModule()
{
	...
	//no bufffer b/c no sample rate yet
	m_pBuffer = NULL;
	m_nBufferSize = 0;
...
*/
bool __stdcall CDLLModule::initialize()
{
	// Add your code here
...
*/
bool __stdcall CDLLModule::prepareForPlay()
{
	// Add your code here:


	//set up delay line
	m_nBufferSize = 5*m_nSampleRate; // x seconds of delay @ fs
	...
	cookVariables();
			...
			//flushing delays from shelf filter
		m_LeftShelf.flushDelays();
		m_RightShelf.flushDelays();
		//calculate intial values for shelf
		calculateShelfCoeffs(m_fBassGain, m_fTrebleGain);
...
*/
bool __stdcall CDLLModule::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	int nReadIndex_1 = m_nReadIndex - m_fDelayInSamples;//(int)m_fDelayInSamples;
	if(nReadIndex_1 < 0)
		nReadIndex_1 = m_nBufferSize- m_fDelayInSamples; // m_nBufferSize-1 is last location
	...
	//2
	//read location TWO BEHIND yn at y(n-)
	int nReadIndex_2 = m_nReadIndex - 2*(int)m_fDelayInSamples;//10000; //can be any number- hard code
	if(nReadIndex_2 <0)
		nReadIndex_2 = m_nBufferSize- 2*(int)m_fDelayInSamples;//10000; //mnBuffersize-[] is last location

	//get y(n-2)
	float yn_2 = m_pBuffer[nReadIndex_2];

	// interpolate (0,yn) and (1, yn_2) by the amount fracDelay
	float fFracDelay_2 = m_fDelayInSamples - (int)m_fDelayInSamples;

	// linerp: x1, x2,y1,y2, x
	float fInterp_2 = dLinTerp (0, 1, yn, yn_2, fFracDelay_2); //interp frac bewteen them


	//3
	//read location THREE BEHIND yn at y(n-3)
	int nReadIndex_3 = m_nReadIndex - 3*(int)m_fDelayInSamples; //
	if(nReadIndex_3 <0)
		nReadIndex_3 = m_nBufferSize- 3*(int)m_fDelayInSamples; //mnBuffersize- [] is last location

		//get y(n-3)
	float yn_3 = m_pBuffer[nReadIndex_3];

	//interpolte (0,yn) and (1, yn_3) by the amount fracDelay
	float fFracDelay_3 = m_fDelayInSamples - (int)m_fDelayInSamples;

	//linerp: x1, x2,y1,y2, x
	float fInterp_3 = dLinTerp (0, 1, yn, yn_3, fFracDelay_3);  //interp frac bewteen them
...
////
	...
	if(m_fDelayInSamples == 0)
		...
		yn = fInterp;
	//yn = yn_1*.333 + yn_2*.333 + yn_3 * .333;
	float prefilter; 
	prefilter = m_fWetLevel*yn + (1.0 - m_fWetLevel)*xn;
	float shelf;
	shelf = m_LeftShelf.doBiQuad(prefilter);
	...
	//1
	// Read the location ONE BEHIND yn at y(n-1)
	int nReadIndex_1 = m_nReadIndex - m_fDelayInSamples;//2*(int)m_fDelayInSamples;
	if(nReadIndex_1 < 0)
		nReadIndex_1 = m_nBufferSize- m_fDelayInSamples; // m_nBufferSize-1 is last location
...
/////////
	...
	if(m_fDelayInSamples == 0)
		...
		yn = fInterp;
		//yn = yn_1*.333 + yn_2*.333 + yn_3 * .333;
		//pOutputBuffer[1] = m_RightShelf.doBiQuad(pInputBuffer[1]);
	float prefilter; 
	prefilter = m_fWetLevel*yn + (1.0 - m_fWetLevel)*xn;
	float shelf;
	shelf = m_RightShelf.doBiQuad(prefilter);
	float f_xn = shelf;
...
//
bool __stdcall CDLLModule::userInterfaceChange(int nControlIndex)
{
	//cook
	cookVariables();
...
*/
bool __stdcall CDLLModule::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
*/
bool __stdcall CDLLModule::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CDLLModule::processVSTAudioBuffer(float** ppInputs, float** ppOutputs,
...
bool __stdcall CDLLModule::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CDLLModule::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CDLLModule::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CDLLModule::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CDLLModule::midiClock()
{
...
bool __stdcall CDLLModule::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
bool __stdcall CDLLModule::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! ------------------------------ //
